...
`ADKAgentEngineConnector` is the seam between them.
...
graph TB
    ...
    subgraph GCP["☁️ Google Cloud"]
        ...
        subgraph APR["🤖 Agent Platform Runtime"]
            Agent[🧠 ADK Agent<br/>async_stream_query]
        end
...
- **Agent** - the ADK agent, hosted on Agent Platform Runtime; deployed from [`agent/`](./agent/)
...
- **Active GCP Project** with billing enabled and access to:
  - **Vertex AI** (Agent Platform Runtime) — enable it before deploying: `gcloud services enable aiplatform.googleapis.com`
  - **Cloud Run**, **Cloud Build**, and **Artifact Registry** (enabled automatically by `server/deploy.sh`)
- **Gemini 3.5 Flash** enabled in [Model Garden](https://console.cloud.google.com/vertex-ai/model-garden)
...
then `deploy-secret`, then `deploy-server`. Or run the steps individually:
`make deploy-agent` / `make deploy-secret` / `make deploy-server`.

**Deployment output:**

```
✅ Deployed: https://tac-server-adk-xxxxx.<region>.run.app

Configure Twilio:
  Conversation Orchestrator status callback : https://tac-server-adk-xxxxx.<region>.run.app/webhook   (POST)
  Voice "A call comes in" webhook           : https://tac-server-adk-xxxxx.<region>.run.app/twiml     (POST)
```
...
```bash
gcloud run services logs read tac-server-adk --project your-project-id --region your-region --limit 50
```

Or go to the [Cloud Logging Console](https://console.cloud.google.com/logs), and filter by **Cloud Run Revision → `tac-server-adk`**.
...
```bash
gcloud logging read \
  'resource.type="cloud_run_revision" AND resource.labels.service_name="tac-server-adk" AND httpRequest.requestUrl!=""' \
...
- After editing the agent: `make deploy-agent`.
