...
SERVICE="${CLOUD_RUN_SERVICE:-tac-server}"
...
# (call the agent). secretAccessor is granted per-secret by secrets.sh.
...
( cd "$REPO_ROOT" && gcloud builds submit \
    --project "$PROJECT" \
    --config deploy/agent_platform/server/cloudbuild.yaml \
