...
hood it produces a source-code app on Agent Platform Runtime (Agent Engine),
the same runtime the [`adk/`](../../adk/) example uses.
...
`query`/`stream_query` methods, so the SDK's dynamically-bound client methods
don't apply here — this is also why `StudioAgentEngineConnector` (used by
`../server/main.py`) talks to it over the same REST endpoint.
