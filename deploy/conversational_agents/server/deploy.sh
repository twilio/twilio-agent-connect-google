...
SERVICE="${CLOUD_RUN_SERVICE:-tac-df-server}"
...
# The Dialogflow API is already enabled for any project that has a
# Conversational Agents agent, so it is not enabled here.
gcloud services enable \
    run.googleapis.com cloudbuild.googleapis.com artifactregistry.googleapis.com \
    --project "$PROJECT" --quiet

# Grant the deploy SA the roles it needs: build (Cloud Build) and runtime
# (call the Dialogflow agent). secretAccessor is granted per-secret above.
for role in roles/cloudbuild.builds.builder roles/dialogflow.client; do
...
( cd "$REPO_ROOT" && gcloud builds submit \
    --project "$PROJECT" \
    --config deploy/conversational_agents/server/cloudbuild.yaml \
