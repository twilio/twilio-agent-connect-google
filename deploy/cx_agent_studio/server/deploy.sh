...
ENV_FILE="$SCRIPT_DIR/../.env"   # cx_agent_studio/.env
...
SERVICE="${CLOUD_RUN_SERVICE:-tac-cx-server}"
...
# The CES API is already enabled for any project that has a CX Agent Studio
# agent, so it is not enabled here.
gcloud services enable \
    run.googleapis.com cloudbuild.googleapis.com artifactregistry.googleapis.com \
    --project "$PROJECT" --quiet

# Grant the deploy SA the roles it needs: build (Cloud Build) and runtime
# (call the CES agent). secretAccessor is granted per-secret above.
for role in roles/cloudbuild.builds.builder roles/ces.client; do
...
( cd "$REPO_ROOT" && gcloud builds submit \
    --project "$PROJECT" \
    --config deploy/cx_agent_studio/server/cloudbuild.yaml \
