"""Example: Connect Twilio to an Agent Studio app deployed on GCP Agent Platform Runtime.

This example shows how to:
1. Connect to a source-code app built in Agent Studio and deployed on GCP
   Agent Platform Runtime (Reasoning Engine)
2. Run a TAC FastAPI server that handles Twilio webhooks
3. Use a session per conversation, reused across turns — same ADK session
   mechanism as the adk/ example, invoked over the `streamQuery` REST
   endpoint since Studio apps aren't registered with SDK-bound methods

Prerequisites:
    - GCP project with Vertex AI enabled
    - Agent built and deployed from the Agent Studio console — see
      deploy/agent_platform/studio/
    - .env file with required environment variables (see .env.example)

Environment Variables (in .env file):
    # GCP Configuration (Agent Studio currently stores and deploys agents in us-west1 only)
